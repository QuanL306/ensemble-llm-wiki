...
Saved to `wiki/syntheses/<slug>.md` with YAML frontmatter (`type: synthesis`). Syntheses are searchable alongside articles and appear in `kb_list_docs` with a `(synthesis)` suffix. Unlike articles, syntheses are written by the AI rather than compiled from source documents — the frontmatter records the originating question and source articles.
